...
def check_steiner(T: nx.Graph, G: nx.Graph, terminals, silent=True):
    ...
    if not silent:
        ...
        assert have_all_nodes_from_instance, "The given T does not have all nodes from G"
        assert have_all_edges_from_instance, "The given T does not have all edges from G"
        assert all_leaves_are_also_terminals, "Not all leaf node is also a terminal node"
